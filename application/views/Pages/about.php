<style>
.background-image {
    background-image: url("<?php echo base_url(); ?>assets/images/bg.png"), linear-gradient(to right, rgba(0, 0, 115, 58%), rgba(66, 15, 15, 88%));
    background-size: cover;
    background-repeat: no-repeat;
    height: 1024px;
    background-blend-mode: overlay;
}

.service-background-img {
    background-image: url("<?php echo base_url(); ?>assets/images/bg.png"), linear-gradient(to right, rgba(0, 0, 115, 58%), rgba(66, 15, 15, 88%));
    background-size: cover;
    background-repeat: no-repeat;
    height: 1024px;
    background-blend-mode: overlay;
}

.bg-services {
    background-image: url("<?php echo base_url(); ?>assets/images/ftbg.png"), linear-gradient(116.34deg, rgba(8, 8, 59, 0.95) -15.85%, #0A0A54 101.18%);
    background-size: cover;
    background-repeat: no-repeat;
    height: 1024px;
    background-blend-mode: overlay;
}

.foot-bg2 {
    background-image: url("<?php echo base_url(); ?>assets/images/fttbg.png"), linear-gradient(to right, rgba(0, 0, 115, 58%), rgba(66, 15, 15, 88%));
    background-size: cover;
    background-repeat: no-repeat;
    height: 840px;
    background-blend-mode: overlay;
}

.footer-bg {
    background-image: url("<?php echo base_url(); ?>assets/images/footbg.png"), linear-gradient(to right, rgba(0, 0, 115, 58%), rgba(66, 15, 15, 88%));
    background-size: cover;
    background-repeat: no-repeat;
    height: 411px;
    background-blend-mode: overlay;
}
</style>

<div class="background-image">
    <div class="container sticky-md-top">
        <div class="container-fluid">
            <header>
                <nav class="navbar navbar-expand-lg navbar-dark fixed-top">
                    <div class="container-fluid">
                        <a class=" navbar-brand logo-margin-right" href="#">
                            <img src="<?php echo base_url(); ?>assets/images/logo.svg" class="d-inline-block align-top">
                        </a>
                        <button class="navbar-toggler" type="button" data-bs-toggle="collapse"
                            data-bs-target="#navbarSupportedContent" aria-controls="toggleMobileMenu"
                            aria-expanded="false">
                            <span class="navbar-toggler-icon"></span>
                        </button>
                        <div class="collapse navbar-collapse" id="navbarSupportedContent">
                            <ul class="navbar-nav transparent-color">
                                <li class="nav-item me-5">
                                    <a class="nav-link active margin-home" href="#banner">HOME</a>
                                </li>
                                <li class="nav-item me-5">
                                    <a class="nav-link active" href="#technologies">TECHNOLOGIES</a>
                                </li>
                                <li class="nav-item me-5 ">
                                    <a class="nav-link active" href="#">ABOUT</a>
                                </li>
                                <li class="nav-item me-5">
                                    <a class="nav-link active" href="#services">SERVICES</a>
                                </li>
                                <li class="nav-item me-5">
                                    <a class="nav-link active" href="#contact">CONTACTS</a>
                                </li>
                                <li>
                                    <form class="d-flex submit-talk">
                                        <button class="btn btn-outline-light margin-btn-talk" type="submit">LETS
                                            TALK</button>
                                    </form>
                                </li>
                            </ul>
                        </div>
                    </div>
                </nav>
            </header>
        </div>
    </div>
    <section id="banner">
        <div class="container">
            <div class="banner-wrapper">
                <div class="banner-cont">
                    <h1>MACH95 SOFTWARE DEVELOPMENT CORP.</h1>
                    <p>We offer you a smart and easy way to build your business</p>
                </div>
            </div>
        </div>
    </section>
    <div class="d-flex justify-content-center" style="margin-bottom: 180px;">
        <button type="button" class="btn btn-more col-4">READ MORE</button>
    </div>
    <div class="slider" id="clients">
        <div class="slide-track">
            <div class="slider slide-size" onclick="window.open('https://yeoubiphilippines.com/', '_blank')"
                style="cursor:pointer;">
                <img src="<?php echo base_url(); ?>assets/images/1.png" style="width:200px ;">
            </div>
            <div class="slider  slide-size" onclick="window.open('https://mdphomes.net/blog/', '_blank')"
                style="cursor:pointer;">
                <img src="<?php echo base_url(); ?>assets/images/2.png">
            </div>
            <div class="slider  slide-size" onclick="window.open('https://fivedads.com/', '_blank')"
                style="cursor:pointer;">
                <img src="<?php echo base_url(); ?>assets/images/3.png">
            </div>
            <div class="slider  slide-size" onclick="window.open('https://shantahl.net/', '_blank')"
                style="cursor:pointer;">
                <img src="<?php echo base_url(); ?>assets/images/4.png">
            </div>
            <div class="slider  slide-size" onclick="window.open('https://tbrincorporated.com/', '_blank')"
                style="cursor:pointer;">
                <img src="<?php echo base_url(); ?>assets/images/5.png">
            </div>
            <div class="slider  slide-size" onclick="window.open('https://gfoxxint.com/info/', '_blank')"
                style="cursor:pointer;">
                <img src="<?php echo base_url(); ?>assets/images/6.png">
            </div>
            <div class="slider  slide-size" onclick="window.open('https://www.homesxpress.net/', '_blank')"
                style="cursor:pointer;">
                <img src="<?php echo base_url(); ?>assets/images/7.png">
            </div>
            <div class="slider  slide-size" onclick="window.open('https://fablife.com.ph/home', '_blank')"
                style="cursor:pointer;">
                <img src="<?php echo base_url(); ?>assets/images/8.png">
            </div>
            <div class="slider slide-size" onclick="window.open('https://bilisbenta.com/', '_blank')"
                style="cursor:pointer;">
                <img src="<?php echo base_url(); ?>assets/images/9.png" style="width:200px ;">
            </div>
            <div class="slider  slide-size" style="cursor:pointer;">
                <img src="<?php echo base_url(); ?>assets/images/10.png" style="width:200px ;">
            </div>
            <!-- Same slide -->
            <div class="slider slide-size" onclick="window.open('https://yeoubiphilippines.com/', '_blank')"
                style="cursor:pointer;">
                <img src="<?php echo base_url(); ?>assets/images/1.png" style="width:200px ;">
            </div>
            <div class="slider  slide-size" onclick="window.open('https://mdphomes.net/blog/', '_blank')"
                style="cursor:pointer;">
                <img src="<?php echo base_url(); ?>assets/images/2.png">
            </div>
            <div class="slider  slide-size" onclick="window.open('https://fivedads.com/', '_blank')"
                style="cursor:pointer;">
                <img src="<?php echo base_url(); ?>assets/images/3.png">
            </div>
            <div class="slider  slide-size" onclick="window.open('https://shantahl.net/', '_blank')"
                style="cursor:pointer;">
                <img src="<?php echo base_url(); ?>assets/images/4.png">
            </div>
            <div class="slider  slide-size" onclick="window.open('https://tbrincorporated.com/', '_blank')"
                style="cursor:pointer;">
                <img src="<?php echo base_url(); ?>assets/images/5.png">
            </div>
            <div class="slider  slide-size" onclick="window.open('https://gfoxxint.com/info/', '_blank')"
                style="cursor:pointer;">
                <img src="<?php echo base_url(); ?>assets/images/6.png">
            </div>
            <div class="slider  slide-size" onclick="window.open('https://www.homesxpress.net/', '_blank')"
                style="cursor:pointer;">
                <img src="<?php echo base_url(); ?>assets/images/7.png">
            </div>
            <div class="slider  slide-size" onclick="window.open('https://fablife.com.ph/home', '_blank')"
                style="cursor:pointer;">
                <img src="<?php echo base_url(); ?>assets/images/8.png">
            </div>
            <div class="slider slide-size" onclick="window.open('https://bilisbenta.com/', '_blank')"
                style="cursor:pointer;">
                <img src="<?php echo base_url(); ?>assets/images/9.png" style="width:200px ;">
            </div>
            <div class="slider  slide-size" style="cursor:pointer;">
                <img src="<?php echo base_url(); ?>assets/images/10.png" style="width:200px ;">
            </div>
        </div>
    </div>
</div>
<section id="technologies" class="mt-10">
    <div class="container">
        <div class="row technologies-header-margin-top">
            <div class="banner-tech">
                <h1 class="text-dark">OUR TECHNOLOGIES</h1>
            </div>
        </div>
        <div class="row technologies-margin-top">
            <div class="align-self-center col-md-6">
                <div class="card-body">
                    <div class=" title-header ">E-commerce</div>
                    <div class="title-body">
                        Our E-commerce systems allow our partners to manage their businesses from the comforts of
                        their
                        homes. Key functionalities include dropshipping, customer/distributor/branch management,
                        inventory management, replicated shops, order/income reports, wallet management, integration
                        to
                        payment channels and basic compensation plan.
                    </div>
                </div>
            </div>
            <div class="col-md-6">
                <div id="video-carousel1" class="carousel" data-bs-ride="carousel">
                    <div class="carousel-inner">
                        <div class="carousel-item active">
                            <div class="card">

                                <div class="card-vid-top">
                                    <iframe width="543" height="362" src="https://www.youtube.com/embed/jNQXAC9IVRw"
                                        title="YouTube video player" frameborder="0"
                                        allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
                                        allowfullscreen></iframe>
                                </div>


                            </div>
                        </div>
                        <div class="carousel-item">
                            <div class="card-vid-top">
                                <iframe width="560" height="315" src="https://www.youtube.com/embed/qmX4C5viy60"
                                    title="YouTube video player" frameborder="0"
                                    allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
                                    allowfullscreen></iframe>
                            </div>
                        </div>
                        <div class="carousel-item">
                            <div class="card-vid-top">
                                <iframe width="543" height="362" src="https://www.youtube.com/embed/jNQXAC9IVRw"
                                    title="YouTube video player" frameborder="0"
                                    allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
                                    allowfullscreen></iframe>
                            </div>
                        </div>
                        <div class="carousel-item">
                            <div class="card-vid-top">
                                <iframe width="543" height="362" src="https://www.youtube.com/embed/jNQXAC9IVRw"
                                    title="YouTube video player" frameborder="0"
                                    allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
                                    allowfullscreen></iframe>
                            </div>
                        </div>
                    </div>
                    <button class="carousel-control-prev" type="button" data-bs-target="#video-carousel1"
                        data-bs-slide="prev">
                        <span class="carousel-control-prev-icon" aria-hidden="true"></span>
                        <span class="visually-hidden">Previous</span>
                    </button>
                    <button class="carousel-control-next" type="button" data-bs-target="#video-carousel"
                        data-bs-slide="next">
                        <span class="carousel-control-next-icon" aria-hidden="true"></span>
                        <span class="visually-hidden">Next</span>
                    </button>
                </div>
            </div>
        </div>
        <div class="row technologies-margin-top">
            <div class="col-md-6">
                <div id="video-carousel2" " class=" carousel" data-bs-ride="carousel">
                    <div class="carousel-inner">
                        <div class="carousel-item active">
                            <div class="card">
                                <div class="card-vid-top">
                                    <iframe width="543" height="362" src="https://www.youtube.com/embed/jNQXAC9IVRw"
                                        title="YouTube video player" frameborder="0"
                                        allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
                                        allowfullscreen></iframe>
                                </div>
                            </div>
                        </div>
                        <div class="carousel-item">
                            <div class="card-vid-top">
                                <iframe width="560" height="315" src="https://www.youtube.com/embed/qmX4C5viy60"
                                    title="YouTube video player" frameborder="0"
                                    allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
                                    allowfullscreen></iframe>
                            </div>
                        </div>
                        <div class="carousel-item">
                            <div class="card-vid-top">
                                <iframe width="543" height="362" src="https://www.youtube.com/embed/jNQXAC9IVRw"
                                    title="YouTube video player" frameborder="0"
                                    allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
                                    allowfullscreen></iframe>
                            </div>
                        </div>
                        <div class="carousel-item">
                            <div class="card-vid-top">
                                <iframe width="543" height="362" src="https://www.youtube.com/embed/jNQXAC9IVRw"
                                    title="YouTube video player" frameborder="0"
                                    allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
                                    allowfullscreen></iframe>
                            </div>
                        </div>
                    </div>
                    <button class="carousel-control-prev" type="button" data-bs-target="#video-carousel2"
                        data-bs-slide=" prev">
                        <span class="carousel-control-prev-icon" aria-hidden="true"></span>
                        <span class="visually-hidden">Previous</span>
                    </button>
                    <button class="carousel-control-next" type="button" data-bs-target="#video-carousel2"
                        data-bs-slide=" next">
                        <span class="carousel-control-next-icon" aria-hidden="true"></span>
                        <span class="visually-hidden">Next</span>
                    </button>
                </div>
            </div>
            <div class="align-self-center col-md-6">
                <div class="card-body">
                    <div class=" title-header">Logistics Management</div>
                    <div class="title-body body-media-margin">
                        Allow your customers to transact seamlessly from their devices right to their door. With our
                        logistics systems: set your delivery scope, assign branches, create parcels, track parcels,
                        track collections, and integrate to other systems.
                    </div>
                </div>
            </div>
        </div>
        <div class="row technologies-margin-top technologies-margin-bottom">
            <div class="align-self-center col-md-6">
                <div class="card-body">
                    <div class=" title-header">Multilevel Compensation</div>
                    <div class="title-body">
                        Grow your business by offering your members multiple ways to earn. Multilevel compensation
                        can
                        be
                        paired with our e-commerce systems to support unilevel, binary, and leadership plans.
                    </div>
                </div>
            </div>
            <div class="col-md-6">
                <div id="video-carousel3" " class=" carousel" data-bs-ride="carousel">
                    <div class="carousel-inner">
                        <div class="carousel-item active">
                            <div class="card">
                                <div class="card-vid-top">
                                    <iframe width="543" height="362" src="https://www.youtube.com/embed/jNQXAC9IVRw"
                                        title="YouTube video player" frameborder="0"
                                        allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
                                        allowfullscreen></iframe>
                                </div>
                            </div>
                        </div>
                        <div class="carousel-item">
                            <div class="card-vid-top">
                                <iframe width="560" height="315" src="https://www.youtube.com/embed/qmX4C5viy60"
                                    title="YouTube video player" frameborder="0"
                                    allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
                                    allowfullscreen></iframe>
                            </div>
                        </div>
                        <div class="carousel-item">
                            <div class="card-vid-top">
                                <iframe width="543" height="362" src="https://www.youtube.com/embed/jNQXAC9IVRw"
                                    title="YouTube video player" frameborder="0"
                                    allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
                                    allowfullscreen></iframe>
                            </div>
                        </div>
                        <div class="carousel-item">
                            <div class="card-vid-top">
                                <iframe width="543" height="362" src="https://www.youtube.com/embed/jNQXAC9IVRw"
                                    title="YouTube video player" frameborder="0"
                                    allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
                                    allowfullscreen></iframe>
                            </div>
                        </div>
                    </div>
                    <button class="carousel-control-prev" type="button" data-bs-target="#video-carousel3"
                        data-bs-slide=" prev">
                        <span class="carousel-control-prev-icon" aria-hidden="true"></span>
                        <span class="visually-hidden">Previous</span>
                    </button>
                    <button class="carousel-control-next" type="button" data-bs-target="#video-carousel3"
                        data-bs-slide=" next">
                        <span class="carousel-control-next-icon" aria-hidden="true"></span>
                        <span class="visually-hidden">Next</span>
                    </button>
                </div>
            </div>
        </div>
    </div>
</section>
<section id="services">
    <div class="bg-services">
        <div class="banner-services">
            <h1 class="text-light services-header-margin">SERVICES</h1>
            <div class="container carousel-margin-top">
                <div id="carousel_items" class="carousel" data-bs-ride="carousel">
                    <div class="carousel-inner">
                        <div class="carousel-item active">
                            <div class="card no-grad">
                                <div class="img-wrapper"><img src="<?php echo base_url(); ?>assets/images/wdd.png"
                                        class="d-block w-100" alt="...">
                                </div>
                                <div class="card-body">
                                    <h5 class="card-title">Web Design and Development</h5>
                                    <p class="card-text">With our agile development philosophy, we work through design,
                                        development, testing, and release with a focus on enabling your business as
                                        quickly as possible with utmost quality.</p>
                                </div>
                            </div>
                        </div>
                        <div class="carousel-item">
                            <div class="card no-grad">
                                <div class="img-wrapper"><img src="<?php echo base_url(); ?>assets/images/ams.png"
                                        class="d-block w-100" alt="...">
                                </div>
                                <div class="card-body">
                                    <h5 class="card-title">Application Maintenance and Support</h5>
                                    <p class="card-text">We offer flexible support for customer care, application
                                        maintenance,reports generation, issue resolution, and system enhancements,
                                        prioritizing items most essential to the core of your business.</p>
                                </div>
                            </div>
                        </div>
                        <div class="carousel-item">
                            <div class="card no-grad">
                                <div class="img-wrapper"><img src="<?php echo base_url(); ?>assets/images/si.png"
                                        class="d-block w-100" alt="..."
                                        style="height: 66.04px; width: 52.34px!important;">
                                </div>
                                <div class="card-body">
                                    <h5 class="card-title">System Integration</h5>
                                    <p class="card-text">Seamlessly integrate with your existing systems and other
                                        trusted partners such as ERP software, and payment channels.</p>
                                </div>
                            </div>
                        </div>
                        <div class="carousel-item">
                            <div class="card no-grad">
                                <div class="img-wrapper"><img src="<?php echo base_url(); ?>assets/images/md.png"
                                        class="d-block w-100" alt="..."> </div>
                                <div class="card-body">
                                    <h5 class="card-title">Mobile and Web App Development</h5>
                                    <p class="card-text"> Reach your audience where they are. Enable your business
                                        across multiple platforms.</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    <button class="carousel-control-prev" type="button" data-bs-target="#carousel_items"
                        data-bs-slide="prev">
                        <span class="carousel-control-prev-icon" aria-hidden="true"></span>
                        <span class="visually-hidden">Previous</span>
                    </button>
                    <button class="carousel-control-next" type="button" data-bs-target="#carousel_items"
                        data-bs-slide="next">
                        <span class="carousel-control-next-icon" aria-hidden="true"></span>
                        <span class="visually-hidden">Next</span>
                    </button>
                </div>
            </div>
        </div>
</section>
<section id="contact">
    <div class="foot-bg2">
        <div class="banner-contact">
            <h1 class="text-light">CONTACT US</h1>
        </div>
        <div class="container align-center mt-5 pt-5" id="responsive">
            <div class="row justify-content-center">
                <div class="col-md-6 body-head ">
                    <div class="build-header">Build your project with us!</div>
                    <br>
                    <br>
                    <div class="body-large-r">
                        Where excited to hear your plans.
                    </div>
                </div>
                <div class="col-md-4 margin-responsive ">
                    <form id="form-margin" role="form" method="post"
                        action="<?php echo base_url('Pages/user_post'); ?>">
                        <div class="form-group">
                            <input type="text" name="name" class="form-control white-text  " id="name"
                                placeholder="Enter your name" style="color: white;">
                        </div>
                        <div class="form-group">
                            <input type="email" name="email" class="form-control mt-3 white-text " id="email"
                                placeholder="Enter your email" style="color: white;">
                        </div>
                        <div class="form-group">
                            <input type="text" name="ype_of_service" class="form-control mt-3 white-text "
                                id="type_of_service" placeholder="Type of services you want from us"
                                style="color: white;">
                        </div>
                        <div class="form-group">
                            <textarea class="form-control mt-3 white-text " name="message" id="message" rows="4"
                                placeholder="Message" style="color: white;"></textarea>
                        </div>
                        <br>
                        <br>
                        <form class="center">
                            <button type="button" class="btn btn-submit col-5">SUBMIT</button>
                        </form>
                    </form>
                </div>
            </div>
        </div>
</section>
<section id="footer">
    <div class="footer-bg">
        <div class="box-1">
            <div class="container footer-font">
                <div class="row">
                    <div class="col border-box middle-border">
                        <a class="navbar-brand logo-margin-top" href="#">
                            <img src="<?php echo base_url(); ?>assets/images/logo.svg" width="112.98px" height="31px"
                                style="
                            margin-top: 30px;">
                        </a>
                        <div class="caption-r">We ara a lorem ipsum dolor sit amet, consectetur
                            adipiscing elit, sed do
                            eiusmod tempor incididun.
                        </div>
                        <div class="row">
                            <div class="d-flex flex-row">
                                <a class="navbar-brand " href="#">
                                    <img src="<?php echo base_url(); ?>assets/images/call.png">
                                </a>
                                <div class="col flex-row">
                                    <div class="caption-r">Have a question?</div>
                                    <div class="body-normal-r">63 32 23 60019</div>
                                </div>
                                <div class="col">
                                    <div class="d-flex flex-row">
                                        <a class="navbar-brand p" href="#">
                                            <img src="<?php echo base_url(); ?>assets/images/mail.png">
                                        </a>
                                        <div class="col flex-row">
                                            <div class="caption-r">Connect with us</div>
                                            <div class="body-normal-r">www.mach95.com</div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="col">
                            <div class="d-flex flex-row">
                                <a class="navbar-brand" href="#">
                                    <img src="<?php echo base_url(); ?>assets/images/www.png">
                                </a>
                                <div class="col flex-row">
                                    <div class="caption-r">Contact us at</div>
                                    <div class="body-normal-r" style=" margin-bottom: 20px;">mach95mail@gmail.com</div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="col border-box">
                        <div class="body-large-r top-margin">
                            Grow your business with us!
                        </div>
                        <div class="body-small-r bd-sm-mg">
                            Lets keep your business ahead of competitors. Share us your thoughts
                        </div>
                        <div class="input-group mb-3 btn-media">
                            <input type="text" class="form-control btn-linear" placeholder="Enter your email"
                                aria-label="email" aria-describedby="button-addon2" style=" margin-right: 20px;">
                            <button class="btn btn-outline-secondary btn-grad-small" type="button" id="button-addon2"
                                style="border-radius: 8px;">Submit
                            </button>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="container">
            <div class="footer navbar-expand-lg">
                <div class="collapse justify-content-evenly navbar-collapse" id="footer-nav">
                    <ul class="navbar-nav mb-2 mb-lg-0">
                        <li class="nav-item me-5 ">
                            <a class="nav-link active" aria-current="page" href="#banner"
                                style="color: #8C8C8C;">Home</a>
                        </li>
                        <li class="nav-item me-5 ">
                            <a class="nav-link active" aria-current="page" href="#technologies"
                                style="color: #8C8C8C;">Technologies</a>
                        </li>
                        <li class="nav-item me-5">
                            <a class="nav-link active" aria-current="page" href="#" style="color: #8C8C8C;">About</a>
                        </li>
                        <li class="nav-item me-5">
                            <a class="nav-link active" aria-current="page" href="#services"
                                style="color: #8C8C8C;">Services</a>
                        </li>
                        <li class="nav-item me-5">
                            <a class="nav-link active" aria-current="page" href="#clients"
                                style="color: #8C8C8C;">Clients</a>
                        </li>
                        <li class="nav-item me-5">
                            <a class="nav-link active" aria-current="page" href="#contact"
                                style="color: #8C8C8C;">Contact</a>
                        </li>
                    </ul>
                </div>
                <div class="caption-r tm-align">
                    © Copyright 2019 • Mach 95 Software Development Corp.
                </div>
            </div>
        </div>
    </div>
</section>